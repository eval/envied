...
So while doing a `ENVied.require` in `config/application.rb` would seem perfectly fine, it won't work in the default Rails setup.
...
ENVied.springified_require(:default, ENV['RAILS_ENV'])

#Spring.after_callback do
ENVied.springify do
